...
class UIPrediction:
    ...
    def create_ui(self):
        self.root = Tk()
        self.root.title("Do Minh Nhat - House Price Prediction tool")
        self.root.geometry("1200x750")
        main_panel = PanedWindow(self.root)
        main_panel["bg"] = "yellow"
        main_panel.pack(fill=BOTH, expand=True)

        top_panel = PanedWindow(main_panel, height=40)
        top_panel["bg"] = "blue"
        main_panel.add(top_panel)
        top_panel.pack(fill=X, side=TOP, expand=False)

        font = Font(family="tahoma", size=12)
        title_label = Label(top_panel, text='House Price Prediction', font=font)
        ...
        evaluate_panel = PanedWindow(center_panel, height=350)
        evaluate_panel["bg"] = "cyan"
        center_panel.add(evaluate_panel)
        evaluate_panel.pack(fill=X)

        table_evaluate_panel = PanedWindow(evaluate_panel, height=350)
